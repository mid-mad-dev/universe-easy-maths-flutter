...
class AdminService {
  ...
  Future<List<Map<String, dynamic>>> getQuestions() async {
    ...
    final userIds = rows
        ...
        .toList();
    final adminIds = rows
        .map((e) => e['teacher_admin_id']?.toString())
        .whereType<String>()
        .toSet()
        .toList();
    ...
    for (final row in rows) {
      ...
      if (raw != null && raw.isNotEmpty) {
        final url = await StorageService().imageUrl(
          AppConstants.doubtBucket,
          raw,
        );
  ...
  Future<void> deleteChapter(String chapterId) async {
    ...
    for (final item in lessons as List) {
      ...
      await _removeStorageValue(
        AppConstants.lessonBucket,
        item['thumbnail_url'],
      );
    }
  ...
  Future<List<Map<String, dynamic>>> getLessons() async {
    final data = await supabase
        .from('lessons')
        .select('*, chapters(chapter_name,chapter_number)')
        .order('created_at', ascending: false);
    return (data as List).map((e) => Map<String, dynamic>.from(e)).toList();
  }
  ...
  Future<List<Map<String, dynamic>>> studentQuestions(String userId) async {
    final data = await supabase
        .from('questions')
        .select(
          'id,user_id,question,image_url,answer,answered,chapter_id,lesson_id,teacher_admin_id,created_at,updated_at',
        )
        .eq('user_id', userId)
        .order('created_at', ascending: false);
    return (data as List).map((e) => Map<String, dynamic>.from(e)).toList();
  ...
  Future<List<Map<String, dynamic>>> studentProgress(String userId) async {
    final data = await supabase
        .from('progress')
        .select(
          'id,user_id,lesson_id,chapter_id,seen,watched_percentage,last_watched',
        )
        .eq('user_id', userId)
